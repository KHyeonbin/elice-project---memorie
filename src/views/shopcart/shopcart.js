import { addCommas, convertToNumber, navigate, createNavbar } from '/useful-functions.js';
...
function addAllEvents() {
  ...
  purchaseButton.addEventListener('click', navigate('/ordercomplete'));
}
...
async function insertProductsfromCart() {
  ...
  products.forEach(async (product) => {
    // 객체 destructuring
    const { _id, name, quantity, imageUrl, price } = product;
    ...
    document.querySelector(`#image-${_id}`).addEventListener('click', navigate(`/product/detail?id=${_id}`));

    document.querySelector(`#name-${_id}`).addEventListener('click', navigate(`/product/detail?id=${_id}`));

    document.querySelector(`#plus-${_id}`).addEventListener('click', () => increaseItemQuantity(_id));

    document.querySelector(`#minus-${_id}`).addEventListener('click', () => decreaseItemQuantity(_id));

    document.querySelector(`#quantityInput-${_id}`).addEventListener('change', () => handleQuantityInput(_id));
  });
}
